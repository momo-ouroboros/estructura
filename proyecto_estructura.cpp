...
int main()
{
	int opcion;
	while(true){
		cout<<"1 Ingreso de orden en la pila"<<endl;
		cout<<"2 Leer ordenes"<<endl;
		cout<<"3 Eliminar ordenes"<<endl;
		cout<<"4 Buscar datos de una orden"<<endl;
		cin>>opcion;
		switch(opcion){
			case 1:{
				ingreso();
...
void ingreso(){
	if(pilao==NULL){
		...
		cout<<"Ingrese una descripcion de la orden sin usar espacio"<<endl;
		n =1; 
        pilao->valor = n;
        std::cin >>pilao->desc;	 
	...
	pilaaux=new(pilas);
	cout<<"Ingrese una descripcion de la orden"<<endl;
	cout<<"Mi Direccion "<<pilaaux<<endl;
	n= n+1;
	pilaaux->valor = n;
	std::cin >>pilaaux->desc;	
	cout<<"Dato ingresado correctamente"<<endl;
...
void mostrar(){
	...
	while(pilaaux!=NULL){
		cout<<pilaaux->valor<<" "<<pilaaux->desc<<endl;
